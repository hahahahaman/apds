      ...
      (iter (while t)
        (if (< lo hi)
            (let* ((mid (+ lo (floor (/ (- hi lo) 2))))
                   (elm (elt seq mid)))
              (if (funcall equality-predicate elm val)
                  (return mid)
                  (cond ((funcall comparison-predicate elm val)
                         (setf lo (1+ mid)
...
(defun partition-plz (seq &optional (comp-func #'<))
  (let ((len (length seq)))
    (if (> len 0)
        (let ((key (elt seq (1- len)))
              (i 0))
          (iter (for j from 0 below (1- len))
            (cond ((funcall comp-func (elt seq j) key)
                   (rotatef (elt seq i) (elt seq j))
                   (incf i))))
          (rotatef (elt seq i) (elt seq (1- len)))
          i)
        -1)))

(defun quicksort-plz (seq &optional (comp-func #'<))
  (let ((len (length seq)))
    (if (> len 0)
        (let ((p (partition-plz seq comp-func)))
          (setf seq (concatenate (type-of seq)
                                 (quicksort-plz (subseq seq 0 p) comp-func)
                                 (vector (elt seq p))
                                 (quicksort-plz (subseq seq (1+ p)) comp-func))))))
  seq)
